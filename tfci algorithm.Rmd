...
X4 <- rnorm(n, 0, 1.4)
L <- rnorm(n, 0, 0.7)
X2 <- rnorm(n, -1 + 0.9*X1 + 0.8*L, 1)
X3 <- rnorm(n, -1 + 2.8*X4 + 1.9*L, 1)
X5 <- rnorm(n, -1 + 2.8*X4 + 1.7*L, 1)
X6 <- rnorm(n, -1 + 17.2*X4 + 1.2*L, 1)
...
```{r}
#**The elements of input 'tiers' should be greater than 0.
## only called in fci() [by default:  doPdsep=TRUE]
                   ...
                   tiers = NULL) ## FIXME: verbose : 2 --> qreach(verbose)
{
  ...
  if (any(G)) {
    ...
    for(i in 1:p) {
      for(j in 1:i) if(amat[i,j] == 1 && tiers[j] > tiers[i]) { 
        amat[i,j] <- 2
        amat[j,i] <- 1
      }
    }
...
#                            maj.rule = FALSE, verbose = verbose)
    ...
    if (conservative || maj.rule) {
      ...
      for(i in 1:p) for(j in 1:i) if(G[i,j]==1 && tiers[j] > tiers[i]) { 
        G[i,j] <- 2
        G[j,i] <- 1
      }
      ##################
...
tfci.trial <- tfci(suffStat, indepTest, alpha = 0.05, p = 6, verbose=TRUE, tiers = c(3, 2, 2, 2, 2, 1)) ## estimate a PAG
plot(tfci.trial)
```
