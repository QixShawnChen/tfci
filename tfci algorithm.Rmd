...
change pdsep to "t-pdsep" (In progress)
 ...
 Need to write a program to avoid some specific d-sep.
 (1)Type 1, conditioning set containing vertices in future tiers.
```{r}
      ...
      if (any(an0 <- amat[x, ] != 0)) {
        ...
        for (y in adj.x)
          if( !fixedEdges[x,y] ) {
            ...
            for(i in vertices_non) {  ##the step is to change the 'tiers' value(s) of the non-existing element to NULL.
              tiers.diff.set[i] = NULL
            }
             
            #Still need to consider what if all the values in vector tiers.diff.set are NULL.
            ##------------ change2 ends
