...
1.Replace circles at “later” variables (if they are connected to earlier) with arrowheads (done)
...
4.Modify skeleton to t-skeleton:
(1) still thinking how to do that. It seems like the tskeleton in tpc package is customized for tpc algorithm only. For example, it doesn't consider the stable.fast option in "skel.method" parameter.
(2) Many parameters in skeleton function have been omitted in tskeleton function.

Thoughts:
(1)Do we need to modify any of the ten rules in tfci function? In the default setting of fci function, the parameter rules = rep(TRUE, 10).
...
tfci_step1 <- function(suffStat, indepTest, alpha, labels, p,
                ...
                maj.rule = TRUE, numCores = 1, selectionBias = TRUE,
